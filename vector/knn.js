...
add(id, text, metadata = {}) {
    const { vector, sparse } = this.generator.generateVector(text);
    ...
    this.items.set(id, { text, metadata, vector, sparse, normSq });
...
search(queryText, k = 10) {
    const { vector: queryVec, sparse: qSparse } = this.generator.generateVector(queryText);
    ...
    const qIndices = { word: [], prefix: [], bigram: [], char: [] };
