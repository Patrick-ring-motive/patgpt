...
async function onRequest(request, env) {
  ...
  if (request.url.includes('upsert')) {
    ...
    if (!/[a-z]/i.test(prompt)) return new Response(null, {
      status: 400,
      headers: allowAll()
    });
    ...
    await env.PATGPT_VECTOR_CACHE.upsert([{
      id: key,
      values: normalized
    }]);
  ...
  if (/query|match/.test(request.url)) {
    ...
    const vec = generator.generateVector(prompt);
    const normalized = normalizeL2(vec);
    const matches = await env.PATGPT_VECTOR_CACHE.query(normalized, {
      topK: 1,
      returnValues: true,
      returnMetadata: 'all'
    });
